    ...
    def test(self, dataloader, ntriple, skip_dict, num_ent):
        ...
        with torch.no_grad():
            with tqdm.tqdm(total=ntriple, unit="ex") as bar:
                current_time = 0
                # key -> entity, values: list, IM representations of the co-o relations.
                cache_IM = {}
                for src_batch, rel_batch, dst_batch, time_batch in dataloader:
                    ...
                    if self.args.IM:
                        ...
                        if current_time != time:
                            ...
                            for k, v in cache_IM.items():
                                ...
                                self.model.agent.update_entity_embedding(
                                    k, ims, self.args.mu
                                )
                            cache_IM = {}

                        if (
                            src not in self.train_entities
                            and rel in self.RelEntCooccurrence["subject"].keys()
                        ):
                            im = self.model.agent.get_im_embedding(
                                list(self.RelEntCooccurrence["subject"][rel])
                            )
                            ...
                            # prediction shift
                            self.model.agent.entities_embedding_shift(
                                src, im, self.args.mu
                            )
                    ...
                    current_entities, beam_prob, current_entites_lst = self.model.beam_search(
                        src_batch, time_batch, rel_batch
                    )
                    ...
                    for i in range(batch_size):
                        ...
                        candidate_answers, idx = np.unique(
                            candidate_answers, return_index=True
                        )
                        ...
                        if (
                            self.args.test_inductive
                            and src in self.train_entities
                            and dst in self.train_entities
                        ):
                            continue

                        # a set of ground truth entities
                        ...
                        ranking_raw = self.get_rank(
                            candidate_score, dst, candidate_answers, num_ent
                        )
