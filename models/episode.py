...
class Episode(nn.Module):
    ...
    def forward(self, query_entities, query_timestamps, query_relations):
        ...
        """
        query_entities_embeds = self.agent.ent_embs(
            query_entities, torch.zeros_like(query_timestamps)
        )
        ...
        for t in range(self.path_length):
            ...
            chosen_relation = torch.gather(
                action_space[:, :, 0], dim=1, index=action_id
            ).reshape(action_space.shape[0])
            chosen_entity = torch.gather(
                action_space[:, :, 1], dim=1, index=action_id
            ).reshape(action_space.shape[0])
            chosen_entity_timestamps = torch.gather(
                action_space[:, :, 2], dim=1, index=action_id
            ).reshape(action_space.shape[0])
            ...
            current_entites_lst.append(current_entites)
    ...
    def beam_search(self, query_entities, query_timestamps, query_relations):
        ...
        beam_log_prob, top_k_action_id = torch.topk(
            logits, beam_size, dim=1
        )  # beam_log_prob.shape [batch_size, beam_size]
        beam_log_prob = beam_log_prob.reshape(-1)  # [batch_size * beam_size]

        current_entites = torch.gather(
            action_space[:, :, 1], dim=1, index=top_k_action_id
        ).reshape(
            -1
        )  # [batch_size * beam_size]
        current_timestamps = torch.gather(
            action_space[:, :, 2], dim=1, index=top_k_action_id
        ).reshape(
            -1
        )  # [batch_size * beam_size]
        prev_relations = torch.gather(
            action_space[:, :, 0], dim=1, index=top_k_action_id
        ).reshape(
            -1
        )  # [batch_size * beam_size]
        self.agent.policy_step.hx = self.agent.policy_step.hx.repeat(
            1, 1, beam_size
        ).reshape(
            [batch_size * beam_size, -1]
        )  # [batch_size * beam_size, state_dim]
        if self.config["history_encoder"] == "lstm":
            self.agent.policy_step.cx = self.agent.policy_step.cx.repeat(
                1, 1, beam_size
            ).reshape(
                [batch_size * beam_size, -1]
            )  # [batch_size * beam_size, state_dim]

        beam_tmp = beam_log_prob.repeat([action_space_size, 1]).transpose(
            1, 0
        )  # [batch_size * beam_size, max_action_num]
        ...
        for t in range(1, self.path_length):
            query_timestamps_roll = (
                query_timestamps.repeat(beam_size, 1).permute(1, 0).reshape(-1)
            )
            ...
            beam_tmp += logits
            beam_tmp = beam_tmp.reshape(
                batch_size, -1
            )  # [batch_size, beam_size * max_actions_num]
            ...
            top_k_log_prob, top_k_action_id = torch.topk(
                beam_tmp, beam_size, dim=1
            )  # [batch_size, beam_size]

            offset = top_k_action_id // action_space_size  # [batch_size, beam_size]
            offset = offset.unsqueeze(-1).repeat(
                1, 1, self.config["state_dim"]
            )  # [batch_size, beam_size]
            self.agent.policy_step.hx = torch.gather(hx_tmp, dim=1, index=offset)
            self.agent.policy_step.hx = self.agent.policy_step.hx.reshape(
                [batch_size * beam_size, -1]
            )
            if self.config["history_encoder"] == "lstm":
                self.agent.policy_step.cx = torch.gather(cx_tmp, dim=1, index=offset)
                self.agent.policy_step.cx = self.agent.policy_step.cx.reshape(
                    [batch_size * beam_size, -1]
                )

            current_entites = torch.gather(
                action_space[:, :, 1].reshape(batch_size, -1),
                dim=1,
                index=top_k_action_id,
            ).reshape(-1)
            current_timestamps = torch.gather(
                action_space[:, :, 2].reshape(batch_size, -1),
                dim=1,
                index=top_k_action_id,
            ).reshape(-1)
            prev_relations = torch.gather(
                action_space[:, :, 0].reshape(batch_size, -1),
                dim=1,
                index=top_k_action_id,
            ).reshape(-1)
    ...
    def load_pretrain(self, file1, file2):
        ...
        pretrain_rel_emb_noinv, pretrain_rel_emb_inv = (
            pretrain_rel_emb[: self.num_rel, :],
            pretrain_rel_emb[self.num_rel :, :],
        )

        _, ent_dim = self.agent.ent_embs.ent_embs.weight.data.shape
        _, pretrained_ent_dim = pretrain_ent_emb.shape
        diff = pretrained_ent_dim - ent_dim

        if self.config["data_path"].split("/")[-1] == "YAGO":
            self.agent.ent_embs.ent_embs.weight.data[:-39, :].copy_(
                torch.from_numpy(pretrain_ent_emb[:, :-diff])
            )
        elif self.config["data_path"].split("/")[-1] == "WIKI":
            self.agent.ent_embs.ent_embs.weight.data[:-120, :].copy_(
                torch.from_numpy(pretrain_ent_emb[:, :-diff])
            )
        elif self.config["data_path"].split("/")[-1] == "ICEWS18":
            self.agent.ent_embs.ent_embs.weight.data[:, :].copy_(
                torch.from_numpy(pretrain_ent_emb[:-8076, :-diff])
            )
        else:
            self.agent.ent_embs.ent_embs.weight.data[:-1, :].copy_(
                torch.from_numpy(pretrain_ent_emb[:, :-diff])
            )
        ...
        self.agent.rel_embs.weight.data[: self.num_rel, :].copy_(
            torch.from_numpy(pretrain_rel_emb_noinv)
        )
        self.agent.rel_embs.weight.data[
            self.num_rel + 1 : self.num_rel * 2 + 1, :
        ].copy_(torch.from_numpy(pretrain_rel_emb_inv))
