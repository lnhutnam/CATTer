...
class Env(object):
    def __init__(self, examples, config, state_action_space=None):
        ...
        ----------
        examples
            quadruples (subject, relation, object, timestamps);
        config
            config dict;
        state_action_space, optional
            pre-processed action space;, by default None
        """
        self.config = config
    ...
    def get_state_actions_space_complete(
        self, entity, time, current_=False, max_action_num=None
    ):
    ...
    def next_actions(
        self, entites, times, query_times, max_action_num=200, first_step=False
    ):
        ...
        actions = self.get_padd_actions(
            entites, times, query_times, max_action_num, first_step
        )
    ...
    def get_padd_actions(
        ...
        """
        actions = np.ones(
            (entites.shape[0], max_action_num, 3), dtype=np.dtype("int32")
        )
        actions[:, :, 0] *= self.rPAD
        actions[:, :, 1] *= self.ePAD
        actions[:, :, 2] *= self.tPAD
        for i in range(entites.shape[0]):
            ...
            if times[i] == query_times[i]:
                action_array = self.get_state_actions_space_complete(
                    entites[i], times[i], False
                )
            else:
                action_array = self.get_state_actions_space_complete(
                    entites[i], times[i], True
                )
            ...
            if action_array.shape[0] > (max_action_num - start_idx):
                ...
                actions[
                    i,
                    start_idx:,
                ] = action_array[: max_action_num - start_idx]
            else:
                actions[
                    i,
                    start_idx : action_array.shape[0] + start_idx,
                ] = action_array
        return actions