...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Data preprocess", usage="preprocess_data.py [<args>] [-h | --help]"
    )
    parser.add_argument(
        "--data-dir", default="data/ICEWS14", type=str, help="Path to data."
    )
    parser.add_argument(
        "--outfile",
        default="state_actions_space.pkl",
        type=str,
    ...
    with tqdm(total=len(dataset.allQuadruples)) as bar:
        for head, rel, tail, t in dataset.allQuadruples:
            if (head, t, True) not in state_actions_space.keys():
                state_actions_space[(head, t, True)] = (
                    env.get_state_actions_space_complete(
                        head, t, True, args.store_actions_num
                    )
                )
                state_actions_space[(head, t, False)] = (
                    env.get_state_actions_space_complete(
                        head, t, False, args.store_actions_num
                    )
                )
            if (tail, t, True) not in state_actions_space.keys():
                state_actions_space[(tail, t, True)] = (
                    env.get_state_actions_space_complete(
                        tail, t, True, args.store_actions_num
                    )
                )
                state_actions_space[(tail, t, False)] = (
                    env.get_state_actions_space_complete(
                        tail, t, False, args.store_actions_num
                    )
                )
            bar.update(1)

    pickle.dump(
        state_actions_space, open(os.path.join(
            args.data_dir, args.outfile), "wb")
    )
